...
public class RoutesGenerated extends BaseRestRouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        interceptFrom()
            .process(new OpenApi4jValidator("rarity-sys-api.yaml", contextPath));

        rest()
            .get("/rarity/{collection}/{id}")
                .id("get-rarity-collection-id")
                .produces("application/json")
                .to(direct("get-rarity-collection-id").getUri())
            .post("/rarity/{collection}/{id}")
                .id("post-rarity-collection-id")
                .consumes("application/json")
                .to(direct("post-rarity-collection-id").getUri())
        ;
    }
}
